...
Для начала посмотрим на примеры сообщений разных классов:
...
Распределение длин сообщений в  символах          |  Распределение длин сообщений в словах 
:-------------------------:|:-------------------------:
...
Результат получился лучше случайного, однако этого мало )))
...
Для векторизации будем использовать `tf-idf`, перед этим преобразуем текст: 
...
Видим множество фичей, пересекающихся с нашей гипотизой, про связь  рекламы "зароботка" и "бизнеса" с классом `Spam` - `offer`, `free`, `earn`
...
Таким образом мы снизили размерность embeddings в 435 (!!!) раз (с 43500 до 100), однако целевая матрика `AUC` просела на 0.08 (0.96 -> 0.88)


### CatBoost

Посмотрим, как справятся с векторизацией встроенные методы `CatBoost`
...
Видим, что в данном примере слова `digital currency` и `million` стали определяющими, взглянем на еще пару примеров
